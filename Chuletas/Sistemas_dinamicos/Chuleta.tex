...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item \textit{Tiempo de establecimiento $(t_s)$:} tiempo en el que la salida no se \textit{despega} más de un $5\%$ del valor final. En subamortiguados se ve en la tabla.
		\item \textit{Resonancia, máxima amplitud, en frecuencia $(\omega_r)$:} frecuencia en la que se encuentra el máximo (solo para valores de $\xi < 1/\sqrt{2}$). Pico $M_r = \dfrac{A_{max}}{G(0)} = AdB - G(0)dB = \dfrac{1}{2\xi\sqrt{1-\xi^2}} = \dfrac{1}{\sin2\alpha}$.
